...
namespace TrilloBit3sIndieGames
{
    ...
    public class Barquinho : MonoBehaviour
    {
        ...
        [Header("Direção")]
        public float SteerPower = 0.15f; // 0.5f; velovidade de ratação
        ...
        void ApplySteering()
        {
            ...
            float speedPercent = Mathf.Clamp01(Mathf.Abs(forwardSpeed) / (MaxSpeed + 15f));
            // curva melhor comportamento de barco
            float steerFactor = speedPercent * (1f - speedPercent) * 4f;
            // limite mínimo pra não travar totalmente
            steerFactor = Mathf.Clamp(steerFactor, 0.15f, 1f);
...
namespace TrilloBit3sIndieGames
{
    ...
    public class Barquinho : MonoBehaviour
    {
        ...
        [Header("Direção")]
        public float SteerPower = 1f; // velovidade de ratação
        public float SteerSmooth = 0.01f;
        public float MinSteerSpeed = 1f;
        private float currentSteerTorque;
        ...
        void ApplySteering()
        {
            ...
            if (!Mathf.Approximately(inputSteer, 0f))
            {
                ...
                // SUAVIZAÇÃO (inércia do barco)
                currentSteerTorque = Mathf.Lerp(
                    currentSteerTorque,
                    targetTorque,
                    Time.fixedDeltaTime * 0.5f // ↓ quanto menor, mais lento vira
                );
